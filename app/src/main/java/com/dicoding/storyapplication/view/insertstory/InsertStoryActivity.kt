...
import com.dicoding.storyapplication.data.Result
import com.dicoding.storyapplication.databinding.ActivityInsertStoryBinding
import com.dicoding.storyapplication.utils.createCustomTempFile
import com.dicoding.storyapplication.utils.reduceFileImage
import com.dicoding.storyapplication.utils.rotateFile
import com.dicoding.storyapplication.utils.uriToFile
import com.dicoding.storyapplication.view.ViewModelFactory
import com.dicoding.storyapplication.view.liststory.ListStoryActivity
...
class InsertStoryActivity : AppCompatActivity() {
    ...
    private lateinit var currentPhotoPaths: String
    ...
    private fun setupView() {
        ...
        if (myactionbar != null) {
            myactionbar.title = getString(R.string.add_page)
    ...
    private fun myLocation() {
        if (checkPermission(Manifest.permission.ACCESS_FINE_LOCATION) &&
            ...
            fusedLocationClients.lastLocation.addOnSuccessListener { location: Location? ->
                if (location != null) {
    ...
    private val launcherIntentCamera = registerForActivityResult(
        ...
        if (it.resultCode == RESULT_OK) {
            val myFile = File(currentPhotoPaths)
            myFile.let { file ->
    ...
    private fun startTakePhoto() {
        ...
        createCustomTempFile(application).also {
            ...
            currentPhotoPaths = it.absolutePath
    ...
    private val launcherIntentGallery = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            val selectedImages: Uri = result.data?.data as Uri
            val myFile = uriToFile(selectedImages, this@InsertStoryActivity)
            getFile = myFile
            insertBinding.previewImageView.setImageURI(selectedImages)
        }
    }
        ...
        insertBinding.uploadButton.setOnClickListener {
            if (locationCB.isChecked) {
                if (insertBinding.edAddDescription.text.isNullOrEmpty()) {
                    ...
                    val descriptions = insertBinding.edAddDescription.text.toString()
                    if (!TextUtils.isEmpty(descriptions) && getFile != null && latitude != null && longitude != null) {
                        postStory(descriptions)
                    } else {
                        showAlert(
    ...
    private fun convertImage(): MultipartBody.Part {
        ...
        val requestImageFiles = file.asRequestBody("image/jpeg".toMediaTypeOrNull())
