package com.dicoding.storyapplication.customview
...
import com.dicoding.storyapplication.utils.emailValidation
import com.google.android.material.textfield.TextInputLayout

class MyEmailEditText : AppCompatEditText, View.OnTouchListener {
    private lateinit var clearButtonImages: Drawable
    ...
    private fun init() {
        clearButtonImages = ContextCompat.getDrawable(context, R.drawable.ic_clear) as Drawable
    ...
    private fun showClearButton() {
        setButtonDrawables(endOfTheText = clearButtonImages)
    }
    ...
    override fun onTouch(v: View?, event: MotionEvent): Boolean {
        if (compoundDrawables[2] != null) {
            ...
            if (layoutDirection == View.LAYOUT_DIRECTION_RTL) {
                clearButtonEnd = (clearButtonImages.intrinsicWidth + paddingStart).toFloat()
                when {
                    event.x < clearButtonEnd -> isClearButtonClicked = true
                }
            } else {
                clearButtonStart = (width - paddingEnd - clearButtonImages.intrinsicWidth).toFloat()
                when {
            ...
            if (isClearButtonClicked) {
                when (event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        clearButtonImages = ContextCompat.getDrawable(
                            context,
                            R.drawable.ic_clear
                        ) as Drawable
                        showClearButton()
